...
> - Phase 6 native smoke 门槛已加固：`scripts/feishu/smoke.ts` 支持 `--require-todo120-native`，`smoke-plan` 生成的 check-env / strict-live 命令默认带该开关；缺少第二个 agent bot env、第二个 app id 与主 `FEISHU_APP_ID` 相同、或第二个 app secret 与主 `FEISHU_APP_SECRET` 相同，TODO120 native smoke check-env / strict-live 会在触网前失败；strict-live artifact 会写入安全的 `todo120NativeSmoke` readiness 摘要和 app/tenant hash，`--verify-evidence` 与 AgentSpace final evidence gate 都会拒绝未以 TODO120 native 模式生成、缺少 app identity hash 或 app/tenant hash 不匹配当前 Feishu integration 的 artifact，并在 JSON `summary` 里暴露 TODO120 native ready / required / configured 计数，避免单 bot / 无关 app OpenAPI smoke 被误当成 Phase 6 完成或现场无法判断失败原因。
