...
> - AgentSpace `smoke-plan` 已同步 active binding 前置条件：workspace 只有 disabled / archived Feishu binding 时，绑定、凭据、scope、health、smoke env、chat/user 绑定、auto-provision 和 final evidence 步骤都会保持 blocked / pending，并暴露 `integration_not_active`，避免现场把旧记录当成可用 bot。
